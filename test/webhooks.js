...
var OUTCOMES = {}
var LOADTEST = 0
var WEBHOOKS_READY = false

function bootstrapWebhooks (done, asTest) {
  if (WEBHOOKS_READY) {
    asTest ? done() : done(webHooks)
  } else {
    var fileConfig = {
      filename: 'webHooksDB.json'
    }
    var fileStorage = require('../src/storage').get('file', fileConfig)
...
function handleRequest (request, response) {
  ...
  request.on('data', function (chunk) {
    ...
    if (request.url.indexOf('/loadtest/') !== -1) LOADTEST++
    // debug('body:', body)
...
describe('Tests >', function () {
  ...
  it('eventually delete old storage (DB file)', function (done) {
    try {
  ...
  it('create a node-webHooks instance', function (done) {
    bootstrapWebhooks(done, true)
  })
  ...
  it('should get the webHook using the .getWebHook method', function (done) {
    webHooks.getWebHook('hook1', function (obj) {
      should.exist(obj)
      debug(obj)
  ...
  it('should fire the webHook with custom headers', function (done) {
    bootstrapWebhooks(function (webHooks) {
      OUTCOMES = {}

      webHooks.add('hookWithCustomHeaders', URI + '/10/aaa').then(function () {
        webHooks.trigger('hookWithCustomHeaders', {}, {
          hero: 'hulk'
        })
        this.timeout(3000)
        setTimeout(function () {
          debug('OUTCOME-3:', OUTCOMES)
          should.exist(OUTCOMES['/10/aaa'])
          expect(OUTCOMES['/10/aaa']).to.have.deep.property('headers.hero').equal('hulk')
          expect(OUTCOMES['/10/aaa']).to.have.property('body').equal('{}')
          done()
        }, 1000)
  ...
  it('should create a new webHook called loadtest', function (done) {
    bootstrapWebhooks(function (_webHooks) {
      webHooks.add('loadtest', URI + '/loadtest/aaa').then(function () {
        done()
      }).catch(function (err) {
        throw new Error(err)
      })
  ...
  it('check webHooks were saved successfully using the .getWebHook method', function (done) {
    bootstrapWebhooks(function (_webHooks) {
      _webHooks.add('testcase', URI + '/testcase/aaa').then(function () {
        _webHooks.getWebHook('testcase', function (obj) {
          debug('testcase:', obj)
          should.exist(obj)
          done()
        })
  ...
  it('should fire the webHook 100 times and 100 REST calls are expected from loadtest', function (done) {
    ...
    var loop = setInterval(function () {
      console.log('Got', LOADTEST + '/200', 'REST calls')
      if (LOADTEST === count) {
        clearInterval(loop)
        done()
      }
